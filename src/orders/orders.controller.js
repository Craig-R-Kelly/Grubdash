...
/*  POST "/orders"
    invoked after various validation middleware  */
//  Extract data from req.body.data, defaulting to {}, then extract properties from data.  Construct newOrder with these properties along with a generated ID, and append orders array.  Respond with 201 incl new order.
...
/*  GET "/orders/:orderId"
    invoked after orderExists()  */
//  Respond in JSON with object of specified order thus `{"data":{order object}}`, `res.locals.order` having been set in prior middleware `orderExists()`
function read(req, res) {
  res.json({ data: res.locals.order });
}

/*  PUT "/orders/:orderId"
    invoked after much validation middleware  */
